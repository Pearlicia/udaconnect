...
import os
from models import session, Location, Person
...
class PersonServicer(data_pb2_grpc.PersonServiceServicer):
	...
	def create_person(self, request, context):
		...
		session.commit()
		print(new_person)
		response = data_pb2.Person()
		response.person = True
		return response


class LocationServicer(data_pb2_grpc.LocationServiceServicer):
...
data_pb2_grpc.add_PersonServiceServicer_to_server(
	PersonServicer(), server)

data_pb2_grpc.add_LocationServiceServicer_to_server(
